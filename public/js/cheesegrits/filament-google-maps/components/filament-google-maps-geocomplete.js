function filamentGoogleGeocomplete({
    ...
    return {
        ...
        mapEl: null,
        symbols: {
            "%n": ["street_number", "sublocality_level_2"],
            "%z": ["postal_code"],
            "%S": ["street_address", "route", "political", "sublocality_level_1"],
        ...
        createAutocomplete: function () {
            ...
            let geocompleteOptions = { fields, strictBounds: !1, types },
                geocompleteEl = isLocation
                    ? statePath + "-fgm-address"
                    : statePath,
            ...
            if (geoComplete) {
                ...
                let geoLocate = document.getElementById(
                    statePath + "-geolocate"
                );
                geoLocate &&
        ...
        getReplacements: function (address_components) {
            ...
            return (
                address_components.forEach((component) => {
                    for (let symbol in this.symbols)
                        this.symbols[symbol].indexOf(component.types[0]) !==
                            -1 &&
                            (symbol === symbol.toLowerCase()
                                ? (replacements[symbol] = component.short_name)
                                : (replacements[symbol] = component.long_name));
                }),
                debug && console.log("TEXT TO ADDRESS",replacements),
                replacements
            );
        },
